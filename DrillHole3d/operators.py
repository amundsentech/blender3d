...
class ColorOperator(bpy.types.Operator):
    ...
    bl_idname = "object.color_holes"
    bl_label = "Color Drill Holes"
    ...
    def create_num_attrs(self,columns,obj,data):
        for c in columns:
            try:
                ## update the attributes 
                float_layer = obj.data.attributes.new(name=f'{c}_values',TYPE='FLOAT')
    ...
    def create_color_attrs(self,obj,data,colormaps):
        print('######## CREATE COLOR ATTRIBUTES########')
        
        for c in self.color_cols:
            print(f'float_colors {c}')
            colormap=colormaps[c]
            color_layer =obj.data.attributes.new(name=c,type='FLOAT_COLOR',domain='POINT')
            obj.data.update()
            try:
                ## update the attributes 
                bar=tqdm(data.iterrows())
    ...
    def generate_spline_colors(self,curve,data,render_col,coords,colormaps,name ):
        ...
        shape_key=curve.shape_key_add(name=f'{name} Samples')
        for i, row in data.iterrows():
            bar.update(1)
            coord=coords[i]
            shape_key=curve.shape_key_add(name=f'{name} Samples')
